...
void printDijkstra(int lengthVertices, struct vertex *vertices[lengthVertices]) {
...
int dijkstra(int lengthVertices, int graph[lengthVertices][lengthVertices], int startVertex) {
    ...
    for (int i = 0; i < lengthVertices; i++) {
        ...
        currentVertex->partOfMinGraph = false;
    ...
    while (isEmpty(heap) == false) {
        ...
        extractedVertex->partOfMinGraph = true;
        for (int i = 0; i < lengthVertices; i++) {
            currentVertex = vertices[i];
            if (currentVertex->partOfMinGraph == false) {
    ...
    printDijkstra(lengthVertices, vertices);
    return totalWeight;
}
...
int main() {
    ...
    printGraph(lengthVertices, graph);
    dijkstra(lengthVertices, graph, 0);
