...
float median(int array[], int length) {
    if (length % 2 == 0) {
        ...
        int smallerElement = medianRecursive(array, 0, length - 1, (length - 1) / 2);
        ...
        int largerElement = array[(length / 2)];
        for (int i = (length / 2) + 1; i < length; i++) {
            if (largerElement > array[i]) {
                largerElement = array[i];
            }
        }
        return (float) (smallerElement + largerElement) / 2;
    } else {
